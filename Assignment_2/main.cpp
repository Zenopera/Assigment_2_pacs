...
void performMultiplicationAndMeasureTime( algebra::Matrix<T, Order> &mat, const std::vector<T> &vec, const std::string &description)
{
    Timings::Chrono cron;
    cron.start();
...
int main() {
    ...
    // Perform matrix-vector multiplication in both compressed and uncompressed states
    ...
    // Perform matrix-matrix multiplication in both compressed and uncompressed states
    ...
    try
    {
        auto m = algebra::Matrix<int, algebra::StorageOrder::RowMajor>::readMatrixMarket("lnsp_131.mtx");
        std::vector<int> vec(m.num_Cols(), 1); // Generate a vector of the right dimension

        // Perform matrix-vector multiplication and measure time for different cases
        performMultiplicationAndMeasureTime(m, vec, "Matrix-Vector Multiplication (Uncompressed, Row-Major)");
        performMultiplicationAndMeasureTime(m, vec, "Matrix-Vector Multiplication (Compressed, Row-Major)");
    }
